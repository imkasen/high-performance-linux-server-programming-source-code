...
int main(int argc, char *argv[])
{
    ...
    // 可以预期：每个可能的 socket 连接都可以获得一个这样的对象，
    // 并且 socket 的值可以直接用来索引（作为数组的下标）socket 连接对应的 client_data 对象，
    ...
    while (1) {
        ...
        for (int i = 0; i < user_counter + 1; ++i) {
            if ((fds[i].fd == listenfd) && (fds[i].revents & POLLIN)) {
                ...
                // 对于新的连接，同时修改 fds 和 users 数组。
                // 前文已经提到，users[connfd] 对应于新连接文件描述符 connfd 的客户数据
                user_counter++;
