...
static int pipefd[2];
static sort_timer_lst timer_lst;  // 利用代码清单 11-2 中的升序链表来管理定时器
...
void timer_handler()
{
    timer_lst.tick();  // 定时处理任务，实际上就是调用 tick 函数
    alarm(TIMESLOT);  // 因为一次 alarm 调用只会引起一次 SIGALRM 信号，所以我们要重新定时，以不断触发 SIGALRM 信号
}
